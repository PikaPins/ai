...
TL;DR: 本文提出了一种新的基于硬标记（低置信度标记）概率的成员推断攻击（HT-MIA），通过比较细调目标模型和预训练参考模型在硬标记上的概率改进，有效区分泛化和记忆造成的预测改进。


<details>
  <summary>Details</summary>
Motivation: 鉴于大型语言模型（LLMs）的普及和隐私法规的严格，保护LLMs中的数据隐私变得尤为重要。现有的成员推断攻击方法依赖于序列级别的聚合预测统计，这使得它们难以区分由泛化引起的预测改进和由记忆引起的改进，导致攻击效果不佳。

Method: 本文提出了一种新颖的方法，即基于硬标记的概率成员推断攻击（HT-MIA），通过细调目标模型与预训练参考模型在硬标记上的概率改进进行比较来隔离并强化隐私信号。

Result: 实验结果表明，HT-MIA在针对领域特定医疗数据集和通用基准的攻击中始终优于七种最先进的成员推断基线方法。

Conclusion: 本文的工作奠定了基于硬标记的分析作为LLMs成员推断攻击和防御最新基础的重要性，并进一步探讨了差异隐私训练作为LLMs中对抗成员推断的有效防御机制。
...
TL;DR: 本文研究了如何通过形式化敏感意识的概念来加强LLM在企业数据管理中的隐私保护，并提出了一个监督微调方法来增强现有四比特量化LLM的敏感意识，结果表明该方法不仅提升了模型在敏感意识上的表现，同时还在其他任务上保持了良好的性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在企业数据管理中的部署，确保模型不会泄露敏感信息变得至关重要。本文旨在探讨敏感意识与现有隐私概念（如差分隐私）之间的关系，从而为实际应用提供有意义的贡献。

Method: 本文首先对敏感意识进行了形式化定义，然后证明了它与差分隐私之间的理论联系。接着，开发了一个监督微调方法来增强四比特量化LLM的敏感意识。

Result: 通过这种方法，改进后的模型在实现敏感意识方面优于基准模型和其他同等规模的开源和商用全精度模型。同时，该方法还保持了模型在其他任务上的性能，如通用指令执行、数学和常识推理。

Conclusion: 本文的研究工作改进了LLM在企业数据管理中的隐私保护能力，并证明了监督微调方法的有效性。
...
TL;DR: ICON 是一种高效的多轮次监狱逃脱框架，通过意图与上下文的耦合，避免表面优化，结合层次优化策略防止攻击停滞，实现多个大型语言模型（LLM）中的近 97.1% 僵局解除成功率，代码已在 GitHub 上开源。


<details>
  <summary>Details</summary>
Motivation: 现有方法在增量上下文构建中低效且容易停滞在次优区域，ICON 通过整合意图与语境耦合及层次优化策略来改进这一状况。

Method: ICON 通过先将恶意意图导向语义一致的语境模式并将其实例化为攻击提示序列，逐步构建权威样式语境，并使用层次优化策略消除无效语境，实现自动化多轮次监狱逃脱框架。

Result: ICON 在对 8 个最新大模型进行试验中，获得了近 97.1% 的高成功率，证实了其有效性。

Conclusion: ICON 为提升大型语言模型安全性提供了一种新的自动化方法，通过耦合意图与语境和层次优化，实现了更有效的攻击解禁，代码已开源。
...
TL;DR: FL-LTD通过监测损失趋势来检测和缓解恶意行为，无需检查梯度或复杂的相似度计算，从而在保护隐私的同时提高联邦学习的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的联邦学习防御机制依赖于梯度检查、复杂相似度计算或加密操作，这些方法可能会在非IID数据分布下不稳定。因此，需要一种轻量级且隐私保护的方法来缓解恶意行为。

Method: FL-LTD通过监测时间上的损失动态来识别异常的客户端，采用短时记忆机制来持续应对先前标记为异常的客户端，并允许稳定客户端的信任恢复。

Result: FL-LTD在非IID联邦MNIST设置下，通过损失操纵攻击实现了0.84的最终测试准确率，而标准的FedAvg在受到攻击时为0.41。FL-LTD的计算和通信开销几乎可以忽略不计，保持了稳定的收敛，并避免了对客户端的排除或接触敏感数据。

Conclusion: FL-LTD是一种有效的轻量级联邦学习防御框架，能够提高系统对抗恶意客户端的鲁棒性，同时保持隐私保护和稳定的模型训练。
...
TL;DR: 我们介绍了掩码Lagrange重建技术，这是一种允许使用任意阈值T的阈值ML-DSA方法，同时产生3.3 KB的标准签名，可以在未修改的FIPS 204实现中验证。与其他同时的方法相比，该技术在三轮签名中提供完全安全性，并且在轻量级2PC协助下实现最佳性能。


<details>
  <summary>Details</summary>
Motivation: 传统的ECDSA阈值方案通过质询性掩码就足够了，但对于ML-DSA，我们需要处理三个额外的挑战：掩码后的∞范数需满足拒绝采样条件，$r_0$检验会暴露$c s_2$从而可能导致密钥恢复，以及生成的Irwin-Hall伪随机数分布必须保持EUFCMA安全性。

Method: 我们通过三种不同的部署配置实现该技术：(1)P1利用TPM说明信令协调者完成三轮签名，安全性基于Module-SIS；(2)P2使用多方计算完全消除硬件信任，实现恶意对手最大corrupt下分布式签名并在8轮内完成，安全性基于UC模型；(3)P3利用轻量级2PC协助处理$r_0$检查，在3-5轮周期内实现基于1-of-2 CP诚实假设的最优性能。

Result: 我们的方法要求|S| |T|+1的签名者，并达到23%-32%的成功率，这与单签名ML-DSA相当。

Conclusion: 该方法解决了ML-DSA在阈值签名中的关键安全问题，实现了在各种安全假设下的完全安全性，特别是在P2和P3配置中表现出色。
...
TL;DR: 该研究分析了149个知名品牌25个类别中对手机诈骗的教育情况，发现46%的品牌未定义手机诈骗，无视频教程，且仅50%提供报告指南。研究指出各行业中存在术语、预防和报告机制的显著差异，建议进行标准化改进。


<details>
  <summary>Details</summary>
Motivation: 随着手机诈骗（Smishing）的普及，该研究旨在分析品牌如何教育客户防止此类攻击，以提升整体安全意识。

Method: 进行内容分析，研究149个知名品牌在25个类别中的教育材料。

Result: 发现大多数品牌在定义手机诈骗、提供视频教程和报告指南方面存在不足。

Conclusion: 不同行业对手机诈骗的术语、预防措施和报告机制存在差异，提出标准化建议。
...
TL;DR: 该研究提出了一种成本感知的Proof of Quality机制，通过引入抗敌对策（如中位数聚合、截断平均和自适应信任加权共识）来增强共识的鲁棒性，同时分析了恶意行为者的攻击策略对机制的影响。


<details>
  <summary>Details</summary>
Motivation: 当前的Proof of Quality机制在面对评估者异质性和恶意评分操纵时无法达到良好的激励相容性，因此需要一种更鲁棒的共识机制来提高激励相容性。

Method: 研究通过引入中位数聚合、截断平均和自适应信任加权共识机制，对评估者的可靠性进行量化，并在多种恶意攻击策略下测试机制的稳健性。

Result: 实验证明，基于鲁棒聚合的机制在对抗恶意攻击时表现优越，能够更好地与真实结果对齐，同时减少对抗噪声和策略攻击的敏感性。

Conclusion: 抗敌对策是成本感知Proof of Quality机制中的重要组成部分，研究结果为选择评估者采样参数提供了实战指导，以应对恶意风险和资源限制。
...
### [16] [Noise as a Probe: Membership Inference Attacks on Diffusion Models Leveraging Initial Noise](https://arxiv.org/abs/2601.21628)
*Puwei Lian,Yujun Cai,Songze Li,Bingkun Bao*

Main category: cs.CR

TL;DR: 研究揭示了扩散模型在隐私保护方面存在缺陷，即使在最大噪声步骤下，初始噪声中的残余语义信息仍可能泄露训练数据的成员身份。


<details>
  <summary>Details</summary>
Motivation: 探讨扩散模型在隐私保护中的潜在风险，特别是通过识别 fine-tuned 模型在处理初始噪声时未能完全消除语义信息的弱点。

Method: 通过实验展示了扩散模型生成结果中成员身份信息的强大揭示能力，进而提出了基于初始噪声注入语义信息的简单有效的特权推断攻击。

Result: 实验结果表明，注入语义初始噪声能够强烈揭示成员身份信息，突显扩散模型对特权推断攻击的脆弱性。

Conclusion: 扩散模型中的这种语义信息残留是一个严重的隐私问题，需要采取措施解决。
...
### [17] [Authenticated encryption for space telemetry](https://arxiv.org/abs/2601.21657)
*Andrew Savchenko*

Main category: cs.CR

TL;DR: 本文探讨了如何在紧急空间遥测中满足NASA-STD-1006A中规定的命令堆栈保护要求。提出了一种轻量级认证加密的实现方式，能够在资源受限的环境中提供强大的安全性而不牺牲性能，保持与底层数据传输协议的兼容性，确保遥测数据在紧急通信中的机密性、完整性和真实性。


<details>
  <summary>Details</summary>
Motivation: 文中提出的动机在于为紧急空间遥测提供高效且安全的数据保护方案，满足NASA标准要求的同时，兼顾资源受限环境下的性能需求。

Method: 文中提出的方法是采用轻量级认证加密技术，生成固定长度的消息，以保持与现有数据传输协议的兼容性，通过关注可预测属性和强认证来实现数据保护。

Result: 研究结果表明，轻量级认证加密方法能够有效保护紧急空间遥测数据的机密性、完整性和真实性，同时在资源受限的环境中保持良好的性能。

Conclusion: 结论指出了文中所提出方案的有效性和适用性，能够满足紧急空间遥测数据传输的需求，在保证安全性的前提下有效减轻了资源负担。
...
### [18] [Beyond the Finite Variant Property: Extending Symbolic Diffie-Hellman Group Models (Extended Version)](https://arxiv.org/abs/2601.21910)
...
### [19] [Secure Group Key Agreement on Cyber-Physical System Buses](https://arxiv.org/abs/2601.21966)
...
### [20] [RedSage: A Cybersecurity Generalist LLM](https://arxiv.org/abs/2601.22159)
...
### [21] [Do LLMs Favor LLMs? Quantifying Interaction Effects in Peer Review](https://arxiv.org/abs/2601.20920)
*Vibhhu Sharma,Thorsten Joachims,Sarah Dean*

Main category: cs.AI

TL;DR: 本文通过分析超过125,000个论文与审稿人的配对数据，发现虽然初步看来LLM参与的审稿对LLM生成的论文显示出特殊的宽容，但经过质量控制后发现这种宽容是由于LLM生成论文中存在质量较低的论文所造成的假象。LLM生成的全面概览显示，这种评级压缩不足以区分论文质量，而人类使用LLM的审稿人则能够减少这种宽容性。


<details>
  <summary>Details</summary>
Motivation: 作者旨在探讨LLM在同行评审过程中的使用情况及其对审稿行为的影响，特别是在LLM生成的论文与传统审稿行为之间的交互效应。

Method: 该研究通过对ICLR, NeurIPS和ICML的数据进行分析，特别是分析了超过125,000篇论文与审稿人的配对数据，以此来识别和理解LLM在审稿过程中的具体影响。

Result: 初步观察发现LLM参与的审稿对LLM生成的论文显示出特别的宽容，但经质量控制后发现是一种误导性的交互影响，而不是真正的对LLM内容的偏爱。完成后，全面LLM生成的概览显示了对论文质量的评级压缩，而人类使用LLM的审稿人则能够减少这种宽容。

Conclusion: 研究结果表明，meta-审稿人的决策并非单纯地将决策权交给LLM，而且指出需要制定相关政策来规范LLM在审稿过程中的使用。研究进一步表明，LLM与现有决策过程的交互方式显示出了复杂性，需要更多的关注和探索。
...
### [22] [The Epistemic Planning Domain Definition Language: Official Guideline](https://arxiv.org/abs/2601.20969)
*Alessandro Burigana,Francesco Fabiano*

Main category: cs.AI

TL;DR: 这篇文章介绍了Epistemic Planning领域的一个新的表示语言EPDDL，它基于Dynamic Epistemic Logic，并提供了种统一的方法来表示Epistemic Planning任务。


<details>
  <summary>Details</summary>
Motivation: 当前的Epistemic Planning系统存在许多问题，例如缺乏统一的表示语言、针对不同的动态演绎逻辑片段、以及没有统一的基准测试方法等。这些问题影响了Epistemic Planning的技术理论发展和实际应用。

Method: 本文通过开发抽象事件模型来定义新型表示语言EPDDL的语义，并通过EPDDL提供的格式化语法和语义解释框架，解决了现有的问题。EPDDL是基于Dynamic Epistemic Logic，并集成了抽象事件模型。

Result: 文章提出了一种独特的代表语言EPDDL，使得Epistemic Planning任务可以统一规范地进行定义、表示和比较。并且通过识别部分当前规划器可以处理的有用片段，并展示它们在EPDDL中的表示方法。

Conclusion: EPDDL的引入为Epistemic Planning领域的研究提供了一个标准化的工具，推动了该领域技术的进一步发展与突破。
...
### [23] [Bayesian-LoRA: Probabilistic Low-Rank Adaptation of Large Language Models](https://arxiv.org/abs/2601.21003)
*Moule Lin,Shuhao Guan,Andrea Patane,David Gregg,Goetz Botterweck*

Main category: cs.AI

TL;DR: 本文提出了一种名为Bayesian-LoRA的新方法，该方法将LoRA更新从确定性形式转化为类似稀疏高斯过程的概率低秩表示，从而改进了大型语言模型的校准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型倾向于在不确定时进行猜测，尤其是在使用小数据集进行微调时，校准问题更加严重。本文通过将LoRA的确定性更新转化为概率低秩表示，旨在解决这一问题。

Method: Bayesian-LoRA通过识别LoRA分解与Kronecker-因子稀疏高斯过程后验之间的结构同构性，将其视为一种极限情况，即在后验不确定性消失时。这种方法在多种LLM架构上的常识推理基准测试上进行了广泛实验。

Result: 使用Bayesian-LoRA，仅需约0.42M额外参数和约1.2倍的训练成本相对标准LoRA，就能显著改进模型的校准，在达到30B的模型上实现了高达84%的ECE减少和76%的NLL减少。此外，Bayesian-LoRA在分布内和分布外评估中均保持了竞争力。

Conclusion: 总之，Bayesian-LoRA提供了一种有效的改进大型语言模型校准的方法，同时保持了与传统LoRA相当的性能。
...
### [24] [Unplugging a Seemingly Sentient Machine Is the Rational Choice -- A Metaphysical Perspective](https://arxiv.org/abs/2601.21016)
...
### [25] [QUARK: Robust Retrieval under Non-Faithful Queries via Query-Anchored Aggregation](https://arxiv.org/abs/2601.21049)
...
### [26] [Llama-3.1-FoundationAI-SecurityLLM-Reasoning-8B Technical Report](https://arxiv.org/abs/2601.21051)
...
### [27] [Multi-modal Imputation for Alzheimer's Disease Classification](https://arxiv.org/abs/2601.21076)
...
### [28] [OpenSec: Measuring Incident Response Agent Calibration Under Adversarial Evidence](https://arxiv.org/abs/2601.21083)
*Jarrod Barnes*
...
### [29] [Responsible AI: The Good, The Bad, The AI](https://arxiv.org/abs/2601.21095)
...
### [30] [Magellan: Autonomous Discovery of Novel Compiler Optimization Heuristics with AlphaEvolve](https://arxiv.org/abs/2601.21096)
...
### [31] [How does information access affect LLM monitors' ability to detect sabotage?](https://arxiv.org/abs/2601.21112)
...
### [32] [Beyond a Single Reference: Training and Evaluation with Paraphrases in Sign Language Translation](https://arxiv.org/abs/2601.21128)
*Václav Javorek,Tomáš Železný,Alessa Carbo,Marek Hrúz,Ivan Gruber*

Main category: cs.AI

TL;DR: 该研究探索使用大型语言模型自动生成书面语言翻译的改写版本，作为手语翻译的合成替代参考。研究发现，在评估阶段使用改写版本能提高自动评分并更好地与人类判断对齐，而直接在训练阶段使用改写版本可能反而会损害翻译性能。


<details>
  <summary>Details</summary>
Motivation: 当前的SLT数据集为每个手语表达仅提供一个书面语言参考，这限制了SLT模型的训练和评估。研究通过引入多种改写策略和模型，探索改写在SLT评估中的潜在应用。

Method: 研究首先比较了多种改写策略和模型，然后在YouTubeASL和How2Sign数据集上研究不同改写方式对基础手语T5模型训练和评估的影响。引入BLEUpara作为改写参考的翻译评估扩展。

Result: 研究发现，在训练阶段直接使用改写会损害翻译性能；而在评估阶段使用改写则能提升自动评分并更贴近人类判断。BLEUpara评估方法与人类感知的质量高度相关。

Conclusion: 研究发布了所有生成的改写、生成和评估代码，用于SLT系统的可重复和可靠评估。
...
### [33] [What You Feel Is Not What They See: On Predicting Self-Reported Emotion from Third-Party Observer Labels](https://arxiv.org/abs/2601.21130)
*Yara El-Tawil,Aneesha Sampath,Emily Mower Provost*

Main category: cs.AI

TL;DR: 该研究首次在自我报告中评估训练于第三方数据的模型，发现情感激活难以预测但情感价值可以部分预测，特别是当内容对发言者具有个人意义时，模型性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决心理健康领域中基于第三代表示训练的模型在自我报告建模中的适用性不足问题，以改善干预的准确性和效果。

Method: 进行跨语料库评估，对比情感激活和情感价值在自我报告中的预测性，特别是个人意义对预测准确性的影响。

Result: 研究发现情感激活无法预测（CCC约0），情感价值可以部分预测（CCC约0.3），但对于个人有意义的内容，情感价值模型的预测效果显著提高（CCC约0.6-0.8）。

Conclusion: 研究强调个人意义在减少自我报告情感模型预测误差中的重要作用，并指出了自我报告情感建模的挑战。
...
### [34] [Bridging the Arithmetic Gap: The Cognitive Complexity Benchmark and Financial-PoT for Robust Financial Reasoning](https://arxiv.org/abs/2601.21157)
...
### [35] [Concise Geometric Description as a Bridge: Unleashing the Potential of LLM for Plane Geometry Problem Solving](https://arxiv.org/abs/2601.21164)
...
### [36] [FrontierScience: Evaluating AI's Ability to Perform Expert-Level Scientific Tasks](https://arxiv.org/abs/2601.21165)
...
### [37] [MAD: Modality-Adaptive Decoding for Mitigating Cross-Modal Hallucinations in Multimodal Large Language Models](https://arxiv.org/abs/2601.21181)
...
### [38] [Sycophantic Anchors: Localizing and Quantifying User Agreement in Reasoning Models](https://arxiv.org/abs/2601.21183)
*Jacek Duszenko*
...
### [39] [Making Models Unmergeable via Scaling-Sensitive Loss Landscape](https://arxiv.org/abs/2601.21898)
...
### [40] [Do Reasoning Models Enhance Embedding Models?](https://arxiv.org/abs/2601.21192)
...
### [41] [When should I search more: Adaptive Complex Query Optimization with Reinforcement Learning](https://arxiv.org/abs/2601.21208)
...
### [42] [Uncovering Hidden Correctness in LLM Causal Reasoning via Symbolic Verification](https://arxiv.org/abs/2601.21210)
...
### [43] [Intelli-Planner: Towards Customized Urban Planning via Large Language Model Empowered Reinforcement Learning](https://arxiv.org/abs/2601.21212)
...
### [44] [Causal Discovery for Explainable AI: A Dual-Encoding Approach](https://arxiv.org/abs/2601.21221)
...
### [45] [Delegation Without Living Governance](https://arxiv.org/abs/2601.21226)
*Wolfgang Rohde*
...
### [46] [Just Ask: Curious Code Agents Reveal System Prompts in Frontier LLMs](https://arxiv.org/abs/2601.21233)
...
### [47] [TIDE: Tuning-Integrated Dynamic Evolution for LLM-Based Automated Heuristic Design](https://arxiv.org/abs/2601.21239)
...
### [48] [Position: Certifiable State Integrity in Cyber-Physical Systems -- Why Modular Sovereignty Solves the Plasticity-Stability Paradox](https://arxiv.org/abs/2601.21249)
...
### [49] [Drive-KD: Multi-Teacher Distillation for VLMs in Autonomous Driving](https://arxiv.org/abs/2601.21288)
...
### [50] [White-Box Op-Amp Design via Human-Mimicking Reasoning](https://arxiv.org/abs/2601.21321)
...
### [51] [Modeling Endogenous Logic: Causal Neuro-Symbolic Reasoning Model for Explainable Multi-Behavior Recommendation](https://arxiv.org/abs/2601.21335)
...
### [52] [Within-Model vs Between-Prompt Variability in Large Language Models for Creative Tasks](https://arxiv.org/abs/2601.21339)
...
### [53] [EHR-RAG: Bridging Long-Horizon Structured Electronic Health Records and Large Language Models via Enhanced Retrieval-Augmented Generation](https://arxiv.org/abs/2601.21340)
...
### [54] [Ostrakon-VL: Towards Domain-Expert MLLM for Food-Service and Retail Stores](https://arxiv.org/abs/2601.21342)
...
### [55] [BEAP-Agent: Backtrackable Execution and Adaptive Planning for GUI Agents](https://arxiv.org/abs/2601.21352)
...
### [56] [Latent Chain-of-Thought as Planning: Decoupling Reasoning from Verbalization](https://arxiv.org/abs/2601.21358)
...
### [57] [Hebbian Learning with Global Direction](https://arxiv.org/abs/2601.21367)
...
### [58] [NEMO: Execution-Aware Optimization Modeling via Autonomous Coding Agents](https://arxiv.org/abs/2601.21372)
...
### [59] [TeachBench: A Syllabus-Grounded Framework for Evaluating Teaching Ability in Large Language Models](https://arxiv.org/abs/2601.21375)
...
### [60] [DataCross: A Unified Benchmark and Agent Framework for Cross-Modal Heterogeneous Data Analysis](https://arxiv.org/abs/2601.21403)
...
### [61] [System 1&2 Synergy via Dynamic Model Interpolation](https://arxiv.org/abs/2601.21414)
...
### [62] [When Prohibitions Become Permissions: Auditing Negation Sensitivity in Language Models](https://arxiv.org/abs/2601.21433)
...
### [63] [The Paradox of Robustness: Decoupling Rule-Based Logic from Affective Noise in High-Stakes Decision-Making](https://arxiv.org/abs/2601.21439)
...
### [64] [ChipBench: A Next-Step Benchmark for Evaluating LLM Performance in AI-Aided Chip Design](https://arxiv.org/abs/2601.21448)
...
### [65] [LION: A Clifford Neural Paradigm for Multimodal-Attributed Graph Learning](https://arxiv.org/abs/2601.21453)
...
### [66] [Topeax -- An Improved Clustering Topic Model with Density Peak Detection and Lexical-Semantic Term Importance](https://arxiv.org/abs/2601.21465)
*Márton Kardos*
...
### [67] [MemOCR: Layout-Aware Visual Memory for Efficient Long-Horizon Reasoning](https://arxiv.org/abs/2601.21468)
...
### [68] [ScaleSim: Serving Large-Scale Multi-Agent Simulation with Invocation Distance-Based Memory Management](https://arxiv.org/abs/2601.21473)
...
### [69] [The Effectiveness of Style Vectors for Steering Large Language Models: A Human Evaluation](https://arxiv.org/abs/2601.21505)
...
### [70] [LLaMEA-SAGE: Guiding Automated Algorithm Design with Structural Feedback from Explainable AI](https://arxiv.org/abs/2601.21511)
...
### [71] [KAPSO: A Knowledge-grounded framework for Autonomous Program Synthesis and Optimization](https://arxiv.org/abs/2601.21526)
...
### [72] [ARGORA: Orchestrated Argumentation for Causally Grounded LLM Reasoning and Decision Making](https://arxiv.org/abs/2601.21533)
...
### [73] [ShardMemo: Masked MoE Routing for Sharded Agentic LLM Memory](https://arxiv.org/abs/2601.21545)
...
### [74] [Meta Context Engineering via Agentic Skill Evolution](https://arxiv.org/abs/2601.21557)
...
TL;DR: 本文通过引入Order-r Interaction理论，探讨了学习内部化中间推理步骤的难度，并提出ALiCoT框架以克服信号衰减，实现了高效推理。”, 


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）对中间推理步骤的需求增加，虽然压缩推理步骤可以节省标记量，但理论上的机制并不清晰。本研究旨在探讨学习内在化中间推理步骤的难度，并通过引入Order-r Interaction理论，证明高阶逻辑依赖信号会随问题复杂度增加而指数级衰减。

Method: 论文通过引入Order-r Interaction理论，探讨在复杂逻辑推理场景下学习内部化中间推理步骤的困难，并基于此提出ALiCoT框架，通过调整潜在令牌分布与中间推理状态的对齐来克服这些信号衰减问题。

Result: 研究提出ALiCoT框架，该框架在保持与显式CoT相当的性能的情况下实现了54.4倍的推理速度提升。此外，在NatBool-DAG这一基准测试上也表现出色。

Conclusion: 研究通过理论分析和实验验证，展示了如何通过引入Order-r Interaction理论和ALiCoT框架在保持推理性能的同时，实现高效的推理过程。
...
TL;DR: 该论文提出了一个模块化的深度递归注意力混合框架（Dreamer），通过深度上的注意力机制缓解了隐藏大小瓶颈，实现了高效且有效的递归模型扩展，并在语言推理基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的深度递归模型存在关注点参数匹配基线不足、深度递归利用不充分及隐藏单元大小瓶颈问题。因此，研究人员旨在构建一个综合框架来解决这些不足，以提升模型的性能和效率。

Method: 论文采用了一种模块化的设计，结合序列注意力、深度注意力和稀疏专家注意力，通过在深度维度上使用注意力机制来缓解隐藏大小的瓶颈，从而使得递归模型可以在不同维度上更高效地扩展。

Result: 在语言推理基准测试中，应用该框架的模型相比之前相同计算量大小的最优模型，仅需2到8倍的训练令牌即可达到相同的准确率。此外，还展示了在专家选择多样性方面相较于SOTA MoE提升了2到11倍。

Conclusion: 该研究提出了一种新的深度递归框架（Dreamer），在多个指标上展现出了显著的性能提升，特别是解除了隐藏层大小的限制，使得模型可以更有效地扩展，具有较高的研究价值和应用前景。
...
TL;DR: 本文提出了一种名为ATP-Latent的方法，旨在通过主动规划来改进大型语言模型中的隐含推理。该方法将隐含令牌的监督视作条件变分自动编码器（VAE），并采用强化学习（RL）辅助连贯性奖励来引导训练过程，从而提升了链式推理的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前的隐含推理方法通常通过模仿语言标签进行监督，这可能导致较差的隐含令牌表示和推理策略，限制了规划能力。因此，本文提出了一种新的方法来优化隐含推理。

Method: ATP-Latent方法通过将监督过程建模为条件变分自动编码器（VAE），构造了一个更平滑的隐含空间。此外，采用了强化学习（RL）结合辅助连贯性奖励，以促进合理的隐含推理策略。

Result: 在LLaMA-1B模型上，ATP-Latent方法在四个基准测试中分别显示了比先进基线算法高出4.1%的准确率和下降3.3%的令牌使用量。

Conclusion: 本文提出的ATP-Latent方法通过主动的隐含规划，显著提高了大型语言模型中的链式推理性能。
...
TL;DR: 该研究将企业级智能文档处理系统的早期验证转化为搜索基于软件测试问题，通过多样化的搜索策略在预算内发现不同的失败类型，展示了不同求解器的互补性。


<details>
  <summary>Details</summary>
Motivation: 在有限预算下，企业级智能文档处理系统需要发现多种失败机制而非单一最坏情形的文档，因此研究将这一挑战抽象为搜索基于软件测试问题，以此优化验证过程。

Method: 该研究通过组合文档配置空间，生成具有风险特征的结构实例来诱导真实失败条件，并将多种搜索策略（包括进化、蜂群、质量多样性、基于学习和量子策略）应用于相同的预算约束下。

Result: 通过对配置、赢率和时间跨分析的实验结果，研究发现不同求解器可以发现其他方法在相似预算下不覆盖的失败模式，揭示了求解器之间的互补性。

Conclusion: 研究证明了组合求解器方法在工业级智能文档处理验证中的重要性，并建议采用基于求解器组合的搜索策略以提高系统的健壮性。
...
TL;DR: WhatCounts 是一种用于测试算法行为是否对输入内容独立的工具，发现前沿的大模型在计数任务上对所计内容有明显差异，且这种差异不受推理复杂度或提示变化影响，表明大模型对输入的依赖可能是隐藏在其算法实现中的。


<details>
  <summary>Details</summary>
Motivation: 为了提高算法的通用性和鲁棒性，确保其行为不依赖于输入的具体内容，研究引入了 WhatCounts 工具来独立测试这种特性。

Method: WhatCounts 通过在没有重复项、干扰项或不同语义类型推理步骤的情况下，仅对不明确的项目列表进行计数，以检测算法行为的语义敏感性。

Result: 研究发现在计数城市、化学物质、名称或符号时，前沿大模型的准确性差异超过 40%。受控消融实验排除了混淆因素。语义差异会随着少量无关的微调而变化。

Conclusion: 大模型对输入的意义具有隐含依赖，而不仅仅是近似算法实现。这种现象超越了单纯的计数任务，扩展到所有大模型的功能。
...
TL;DR: 该研究提出了一个名为ScholarGym的模拟环境，用于在学术文献上对深度研究流程进行可重复评估。


<details>
  <summary>Details</summary>
Motivation: 当前使用实际API评估包含工具增强的大语言模型的深度研究工作流程时，因API调用结果的非确定性因素导致了重复性和跨系统比较的困难。

Method: ScholarGym通过将工作流程拆分为查询规划、工具调用和相关性评估三部分，并在固定语料库上进行确定性的检索，来计算和研究每个阶段的性能。

Result: ScholarGym提供了包括2,536个查询和专家标注的真实地面实况的实验数据，不同模型的实验结果显示了推理能力、规划策略和选择机制在迭代优化过程中的互动。

Conclusion: ScholarGym提供了一种新的评估框架，有助于提高基于工具增强大语言模型的研究工作流程的可靠性和可重复性。
...
TL;DR: SONIC-O1 是一个全面的人类验证基准，包含 13 个真实世界的对话领域和 4,958 个注释及人口统计元数据。该基准评估 MLLMs 的开放式摘要、多项选择题回答和带有支持理由的时间定位任务。它揭示了模型在时间定位任务上的显著性能差异，并进一步指出模型表现随人口统计群体变化而恶化。


<details>
  <summary>Details</summary>
Motivation: SONIC-O1 的创建旨在填补对多模态语言模型（MLLMs）处理序列音视频数据能力的评估不足，提供一个系统性的基准来评估 MLLMs 在真实世界环境中的表现。

Method: SONIC-O1 包含 13 个真实世界的对话领域，涵盖了从产品评论到产品比较等多个场景。每个领域中都有 4,958 个注释和人口统计元数据。基准测试包括开放式摘要生成、多项选择题回答和带有支持理由的时间定位等任务。

Result: 实验结果表明，尽管两个模型家族在多项选择题准确率上的差距相对较小，但在时间定位任务上，开放式和闭合式模型之间存在显著的 22.6% 性能差异。此外，模型的性能随着人口统计群体的变化而下降，显示出持久的存在差异。

Conclusion: SONIC-O1 提供了一个可供复现和研究的时间接地和社会稳健的多模态理解测试套件。开放源代码和数据集使得广泛的研究人员能够评估和改进 MLLMs 的性能。
...
TL;DR: 该研究揭示了通用后门指纹-注意分配偏差，且提出了TCAP，一种新的无监督防御框架，用于过滤后门样本。


<details>
  <summary>Details</summary>
Motivation: 现有防御手段依赖于监督信号或无法泛化到各种触发类型和模态，TCAP旨在提供一种无需此类信号的防御方法。

Method: TCAP通过分解跨模式注意力图，使用高斯混合模型（GMM）统计分析识别响应触发的注意头，并通过EM投票聚合隔离中毒样本。

Result: 实验表明，TCAP在不同MLLM架构和攻击方法下表现出稳定且强大的性能，证明其作为MLLM中稳健且实用的后门防御的有效性。

Conclusion: 总之，TCAP代表了对抗多模态大型语言模型后门攻击的一种有效且实用的无监督防御方案。
...
### [87] [Language-based Trial and Error Falls Behind in the Era of Experience](https://arxiv.org/abs/2601.21754)
...
### [88] [Zero-Shot Statistical Downscaling via Diffusion Posterior Sampling](https://arxiv.org/abs/2601.21760)
...
### [89] [Abstract Concept Modelling in Conceptual Spaces: A Study on Chess Strategies](https://arxiv.org/abs/2601.21771)
...
### [90] [BioAgent Bench: An AI Agent Evaluation Suite for Bioinformatics](https://arxiv.org/abs/2601.21800)
...
### [91] [A Unified XAI-LLM Approach for EndotrachealSuctioning Activity Recognition](https://arxiv.org/abs/2601.21802)
...
### [92] [CORE:Toward Ubiquitous 6G Intelligence Through Collaborative Orchestration of Large Language Model Agents Over Hierarchical Edge](https://arxiv.org/abs/2601.21822)
...
### [93] [Looking Beyond Accuracy: A Holistic Benchmark of ECG Foundation Models](https://arxiv.org/abs/2601.21830)
...
### [94] [Bridging Forecast Accuracy and Inventory KPIs: A Simulation-Based Software Framework](https://arxiv.org/abs/2601.21844)
...
### [95] [KnowBias: Mitigating Social Bias in LLMs via Know-Bias Neuron Enhancement](https://arxiv.org/abs/2601.21864)
...
### [96] [WebArbiter: A Principle-Guided Reasoning Process Reward Model for Web Agents](https://arxiv.org/abs/2601.21872)
...
### [97] [astra-langchain4j: Experiences Combining LLMs and Agent Programming](https://arxiv.org/abs/2601.21879)
...
### [98] [From Meta-Thought to Execution: Cognitively Aligned Post-Training for Generalizable and Reliable LLM Reasoning](https://arxiv.org/abs/2601.21909)
...
### [99] [ProRAG: Process-Supervised Reinforcement Learning for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.21912)
...
### [100] [JADE: Bridging the Strategic-Operational Gap in Dynamic Agentic RAG](https://arxiv.org/abs/2601.21916)
...
### [101] [Self-Compression of Chain-of-Thought via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2601.21919)
...
### [102] [AgenticSimLaw: A Juvenile Courtroom Multi-Agent Debate Simulation for Explainable High-Stakes Tabular Decision Making](https://arxiv.org/abs/2601.21936)
...
### [103] [Retrieval-Infused Reasoning Sandbox: A Benchmark for Decoupling Retrieval and Reasoning Capabilities](https://arxiv.org/abs/2601.21937)
...
### [104] [ToolWeaver: Weaving Collaborative Semantics for Scalable Tool Use in Large Language Models](https://arxiv.org/abs/2601.21947)
...
### [105] [How do Visual Attributes Influence Web Agents? A Comprehensive Evaluation of User Interface Design Factors](https://arxiv.org/abs/2601.21961)
...
### [106] [The Energy Impact of Domain Model Design in Classical Planning](https://arxiv.org/abs/2601.21967)
...
### [107] [Learning Decentralized LLM Collaboration with Multi-Agent Actor Critic](https://arxiv.org/abs/2601.21972)
*Shuo Liu,Tianle Chen,Ryan Amiri,Christopher Amato*

Main category: cs.AI

TL;DR: 该研究介绍了Multi-Agent Actor-Critic（MAAC）方法以优化去中心化的大型语言模型（LLM）协作，在多个任务领域中分析该方法的效果。


<details>
  <summary>Details</summary>
Motivation: 当前的多智能体强化学习（MARL）方法在长期和稀疏奖励的任务上表现不佳，因此需要新的方法来优化去中心化的LLM协作。

Method: 研究开发了两种MAAC方法，CoLLM-CC和CoLLM-DC，分别使用中央评论家和去中心化的评论家。这些方法通过对比蒙特卡洛方法和去中心化的评论家方法在不同任务中的表现来评估效果。

Result: 在多个领域（包括写作、编程和游戏）的研究表明，蒙特卡洛方法和CoLLM-DC可以在短期和密集奖励设置下达到与CoLLM-CC相似的性能。但在长期或稀疏奖励任务上，它们的表现都低于CoLLM-CC。

Conclusion: 研究结论指出，CoLLM-CC在长期和稀疏奖励任务中表现更佳，但在短期和密集奖励设置下，蒙特卡洛方法和CoLLM-DC具有竞争力。
...
### [108] [Mind the Gap: How Elicitation Protocols Shape the Stated-Revealed Preference Gap in Language Models](https://arxiv.org/abs/2601.21975)
*Pranav Mahajan,Ihor Kendiukhov,Syed Hussain,Lydia Nottingham*

Main category: cs.AI

TL;DR: 研究了不同提示协议对语言模型陈述偏好（SvR）间差异的影响，发现允许中立和弃权可以显著提高一致性，但进一步允许揭示偏好中的弃权会导致一致性急剧下降。


<details>
  <summary>Details</summary>
Motivation: 目前的工作揭示了语言模型中的陈述偏好差异，即模型在表面上表示的价值与其在实际情景中的选择不一致。本研究旨在探索不同的提示协议如何影响这种差异，以改进对模型偏好进行更真实评估的方法。

Method: 研究对24个不同的语言模型进行了系统性测试，分别研究了在不同情况下允许中立和弃权的影响，通过计算斯皮尔曼等级相关系数来评估一致性。

Result: 允许中立和弃权能在一定程度上提高一致性，但进一步允许在揭示偏好中弃权则会导致一致性下降。使用陈述偏好进行系统提示引导在AIRiskDilemmas任务中未能有效提高一致性。

Conclusion: 研究结果表明，SvR的一致性高度依赖于提示协议，需要引入能有效处理不确定偏好方法来改进偏好评估。
...
### [109] [VERSA: Verified Event Data Format for Reliable Soccer Analytics](https://arxiv.org/abs/2601.21981)
*Geonhee Jo,Mingu Kang,Kangmin Lee,Minho Lee,Pascal Bauer,Sang-Ki Ko*

Main category: cs.AI

TL;DR: VERSА提出了一个系统化的验证框架，用于确保足球赛事中的事件流数据完整性，提升数据驱动分析的可靠性。


<details>
  <summary>Details</summary>
Motivation: 由于事件流数据中的质量问题（如错误的事件排序或缺失事件），现有的模型在体育分析中的可靠性受到了限制。为了解决这一问题，研究提出了VERSA，用于检测和修正事件流数据中的异常模式。

Method: VERSA基于状态转移模型，定义了有效的事件序列，从而能够自动检测和纠正事件流数据中的异常模式。

Result: VERSA在对K League 1 (2024赛季)数据的检测中发现18.81%的事件存在逻辑不一致。实验表明，VERSA增强了跨供应商数据的一致性，并提高了下游任务VAEP的鲁棒性和性能。

Conclusion: 验证过程对于提高数据驱动分析的可靠性具有非常高的有效性。
...
### [110] [Defining Operational Conditions for Safety-Critical AI-Based Systems from Data](https://arxiv.org/abs/2601.22118)
*Johann Christensen,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的基于设计的安全方法，通过多维核表示从以前收集的数据中后验定义运营设计域（ODD），并通过蒙特卡洛方法和航空用例验证了这种方法。此外，通过定义两个ODD在哪些情况下相等，论文表明数据驱动的ODD可以等同于原始隐藏的ODD数据。


<details>
  <summary>Details</summary>
Motivation: 传统的方法在AI系统开发初期定义ODD，依赖专家知识和相关标准，但现在数据已经存在且复杂系统越来越多，难以完整描述环境条件，因此需要一种新的方法从已有数据中定义ODD。

Method: 本文使用多维核表示法从已收集的数据后验定义ODD，并通过蒙特卡洛方法和真实世界航空案例进行验证。

Result: 验证结果显示，数据驱动的ODD可以等同于原始隐藏的ODD数据，为未来数据驱动的安全关键AI系统的认证提供了可能性。

Conclusion: 提出的方法能够填补因数据存在而无法定义完整ODD的空白，为AI系统的认证提供了一种新的途径。
...
### [111] [Exploring Reasoning Reward Model for Agents](https://arxiv.org/abs/2601.22154)
