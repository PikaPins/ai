...
TL;DR: 该论文提出了一种名为 Prefix-Shared KV Cache (PSKV) 的插件式优化技术，显著提高了对大型语言模型（LLMs）的后缀劫持攻击效率，减少了计算时间和峰值内存使用量。


<details>
  <summary>Details</summary>
Motivation: 当前的后缀劫持攻击由于需要评估大量候选后缀，计算成本非常高。观察到这些候选后缀在前缀相同的情况下，PSKV 直接维护一个前缀的单个 KV 缓存，供所有候选后缀共享，从而实现了并行推理。

Method: PSKV 通过缓存共享前缀的 KV，减少重复的前缀推理，利用更加高效的批处理策略，来优化后缀生成过程。

Result: 在六种常用后缀攻击中对五种广泛部署的大型语言模型进行的实验表明，PSKV 能将推理时间减少 40%，峰值内存使用减少 50%，同时保持原始攻击成功率。

Conclusion: PSKV 是一种有效的优化方法，能够显著提升后缀劫持攻击的效率，同时也保持了攻击的效果。
...
TL;DR: 该研究通过使用差分隐私技术生成隐私保护的合成数据，成功地在保障隐私的前提下，实现了敏感金融数据在公共卫生研究中的安全利用。研究结果表明，该合成数据具有时空和预测能力，为流行病学决策提供了支持。研究提出了六种可重用的工具和支持框架，推动了数据共享和隐私保护的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索如何在保持隐私安全的前提下，利用敏感的私有数据进行社会影响研究，特别是对流行病管理的支持。

Method: 研究采用了差分隐私技术，以生成隐私保护的合成金融交易数据，并将其与公共健康和移动性数据集结合，进行分析。

Result: 研究结果显示，这些合成数据具有显著的时空和预测能力，支持流行病学决策，并开发了六种工具和框架，用于诊断预测和前瞻性预测。

Conclusion: 研究提供了在隐私保护基础上进行数据共享的最佳实践，并为公共卫生和移动性分析提供了新的工具和技术支持。
...
TL;DR: 该研究提出了一种新型框架，无需先验触发器知识或干净参考模型，即可净化指令调优的大语言模型。通过构建多种合成的带有恶意触发器-行为配对的大语言模型变体，并与清洁版本对比，揭示共享的'后门签名'。基于此签名，去除可疑组件并进行轻量级微调，生成具有多种后门攻击防御能力的净化模型。


<details>
  <summary>Details</summary>
Motivation: 现有的后门攻击移除方法通常依赖于先验触发器知识、清洁参考模型或激进微调，且主要适用于分类任务。但在现实的指令调校大语言模型环境中，这些假设不再成立，因此需要一种新的框架来更好地处理这类问题。

Method: 提出了一种免疫启发的消除方法，通过构建多种带有不同恶意触发器-行为配对的合成模型变体，并与清洁版本对比，发现共享的'后门签名'。利用这一签名，去除模型中的可疑组件，并进行轻量级微调。

Result: 该研究提出了新的框架，可以在不需要任何先验触发器知识或_CLEAN参考模型的情况下净化指令调优大语言模型。该框架通过构建多种恶意触发器变种模型，与清洁版本进行对比，发现了共享的'后门签名'。基于这一签名，研究者能够去除大语言模型中的可疑组件，并进行轻量级微调，生成具备多种后门攻击防御能力的净化模型。

Conclusion: 该研究提出并验证了一种新的大语言模型后门攻击净化框架，该框架表现出了在现实指令调优环境下对多种后门攻击的有效防御能力，保持了模型的生成能力。
...
TL;DR: 本文提出了一种基于量子波动和启发式旋转轮的混合随机数生成器，用于提高图像加密的安全性。实验结果表明，该随机数生成器有非常好的熵、高非相关性和低峰值信噪比，适用于移动医疗服务等应用。


<details>
  <summary>Details</summary>
Motivation: 随着图像数据在现实生活中的重要性增加，需要更安全的图像传输方法。传统的加密技术依赖于高质量的随机数作为加密密钥，而本文提出的方法旨在通过结合量子技术和启发式算法来改进随机数的质量。

Method: 通过结合量子波动的不确定性原理和启发式算法的有序性，设计并实现了一种混合随机数生成器。该方法首先通过时间演化量子踢动振子模型生成量子波动，然后在启发式的旋转盘上使用这些波动来打乱0到255之间的整数。根据不同位置获取数字序列，并通过动态调整旋转速度来增强不可预测性。

Result: 实验结果显示，该随机数序列具有理想值8的熵、平滑的直方图、极低的相关性，有利于提高数据的不可预测性。将其应用于图像加密并进行加密图像的加密分析，结果显示熵接近理想值，非相关性和PSNR极低，所有方向的相关性都很低。

Conclusion: 本文提出的基于量子和启发式算法的混合随机数生成器，能够在实现高效安全的图像数据保护方面取得良好效果，适合应用于移动健康影像、生物认证等多个场景。
...
TL;DR: 本文强调了GPU滥用是智能交通系统和自动驾驶应用中的一个重要盲点，未经管理的GPU负载会无声地降低实时性能，强调需要加强安全措施以确保系统安全性。


<details>
  <summary>Details</summary>
Motivation: 随着智能交通系统和自动驾驶应用的广泛应用，GPU作为这些系统的关键组成部分，其运行状态却往往缺乏有效的监测。未管理的GPU工作负载可能导致性能下降，增加交通安全和系统安全的风险，因此提出对GPU进行更为严格的监控和管理的必要性。

Method: 文章没有详细描述研究的具体方法，但暗示了通过识别GPU滥用的情况来强调加强安全措施的需求。

Result: 文章指出，未管理的GPU工作负载会无声地降低ITS的实时性能，并提出了需要更强的安全措施来应对这种威胁。

Conclusion: 结论是强调了在智能交通系统和自动驾驶应用中加强对GPU的监控和管理以提高系统的安全性和可靠性的重要性。
...
### [6] [REAEDP: Entropy-Calibrated Differentially Private Data Release with Formal Guarantees and Attack-Based Evaluation](https://arxiv.org/abs/2603.13709)
*Bo Ma,Jinsong Wu,Wei Qi Yan*

Main category: cs.CR

TL;DR: REAEDP 提出了一个结合熵校准直方图发布、合成数据发布机制和攻击评估的差分隐私框架，通过实验验证了其在实际应用中的隐私保护与实用性。


<details>
  <summary>Details</summary>
Motivation: 为了应对敏感数据发布的输出侧隐私威胁（如成员推理、属性推理和记录链接），REAEADP 提供了一种差分隐私框架，旨在在保证隐私的同时，仍然在一定程度上保留数据的实用性。

Method: 该框架结合了熵校准直方图发布、合成数据发布机制，并通过攻击评估进行校验，同时满足差分隐私的正式保障。

Result: 实验表明，在测试范围内，实际熵变化低于理论界限，Laplace 和 Gaussian 基线显示相似趋势，同时隐私参数减少时，成员推理和链接式攻击的效果接近随机猜测。

Conclusion: REAEADP 在实际应用环境中提供了一种实用的隐私保护数据发布管道。
...
### [7] [TableMark: A Multi-bit Watermark for Synthetic Tabular Data](https://arxiv.org/abs/2603.13722)
*Yuyang Xia,Yaoqiang Xu,Chen Qian,Yang Li,Guoliang Li,Jianhua Feng*

Main category: cs.CR

TL;DR: TableMark是一种多比特水印方案，用于增强合成表格数据的溯源性和鲁棒性，同时保持数据的高可用性。


<details>
  <summary>Details</summary>
Motivation: 设计TableMark的原因是为了解决传统水印技术在合成表格数据应用中的挑战，这些问题包括水印在数据重新排序或删除操作后丢失，以及在保证数据可用性和鲁棒性的同时提供高级溯源性。

Method: TableMark将水印编码到合成的表格数据中，通过将水印编码过程形式化为一个具约束的优化问题来实现。这种方法允许数据所有者在数据的鲁棒性和可用性之间进行有效权衡。此外，还提出有效的优化机制以提高数据的可用性。

Result: 实验结果表明，TableMark能够追踪大量用户，具有强鲁棒性，保持高数据可用性。与现有的最佳表水印方案相比，TableMark表现出显著的优势。

Conclusion: 最终，TableMark被证明是一种有效的解决方案，可以解决合成表格数据水印技术中的主要挑战，提供高级的追踪功能和鲁棒性，同时保持数据的可用性。
...
### [8] [Unlinkability and History Preserving Bisimilarity](https://arxiv.org/abs/2603.13735)
*Clément Aubert,Ross Horne,Christian Johansen,Sjouke Mauw*

Main category: cs.CR

TL;DR: 本研究聚焦于'不可链接性'的隐私属性，通过分析设计选择、执行选择与并发敏感性，发现常用的并发语义——历史保持bisimilarity，能够检测到使用分支时间等价性不可见的攻击，且所有研究的协议均适用。我们还提出了一种新型模态逻辑来正式验证隐私属性中的攻击。


<details>
  <summary>Details</summary>
Motivation: 随着关键基础设施越来越依赖于安全协议，确保这些协议能够满足各种要求的重要性日益凸显，尤其是验证隐私属性方面。本研究旨在通过关注'不可链接性'这一隐私属性，探究不同设计选择、执行选择及并发语义是如何影响攻击检测能力的。

Method: 本研究从观察合理的设计选择可能使某些形式化方法未能检测到攻击入手，通过分析追溯等价性在发现攻击上的局限性，使用经典并行语义和由非交织应用π-演算定义的历史保持bisimilarity方法，发现后者能够覆盖所有研究的协议中的攻击。研究还设计了一种新的模态逻辑来描述和验证这些攻击。

Result: 研究表明，历史保持bisimilarity能够发现使用分支时间等价性无法检测到的攻击，这适用于所有研究的协议。此外，研究提出了新的模态逻辑，使其能被用于正式验证隐私属性中的攻击。

Conclusion: 研究结论指出，合理的选择和适当的并发语义对于有效检测隐私协议中的攻击至关重要。历史保持bisimilarity作为一种并发语义，为不可链接性的隐私属性提供了有力的验证工具，而新提出的模态逻辑则为正式验证隐私属性中的攻击提供了理论基础。
...
### [9] [Sirens' Whisper: Inaudible Near-Ultrasonic Jailbreaks of Speech-Driven LLMs](https://arxiv.org/abs/2603.13847)
*Zijian Ling,Pingyi Hu,Xiuyong Gao,Xiaojing Ma,Man Zhou,Jun Feng,Songfeng Lu,Dongmei Zhang,Bin Benjamin Zhu*

Main category: cs.CR

TL;DR: SWhisper 是首个利用常见硬件在现实黑盒条件下对语音驱动的大语言模型进行秘密提示攻击的框架，通过将任意目标基带音频编码到近超声波中，实现隐蔽且不易被察觉的音频传输。


<details>
  <summary>Details</summary>
Motivation: 随着语音逐渐成为访问大语言模型的主要方式，存在新的安全风险，SWhisper 框架旨在应对这些风险。

Method: SWhisper 通过将目标音频编码到近超声波中，利用非线性通道特性和轻量级通道逆补偿，实现隐蔽的音频传输。

Result: 实验表明 SWhisper 在商业和开源语音驱动的大语言模型上的黑盒有效性显著。在商业模型上，SWhisper 达到了 0.94 的非拒绝率 (NR) 和 0.925 的具体说服力 (SC)，且被注入的突破语音对人类听者来说是可感知上不可辨别的。

Conclusion: SWhisper 框架展示了基于语音的秘密提示注入和命令执行攻击的能力，凸显了在语音驱动的接口中加强安全防护的重要性。
...
### [10] [Inevitable Encounters: Backdoor Attacks Involving Lossy Compression](https://arxiv.org/abs/2603.13864)
*Qian Li,Yunuo Chen,Yuntian Chen*

Main category: cs.CR

TL;DR: 论文通过分析损失性压缩对RGB图像中植入触发器的影响，提出了两种针对不可避免损失性压缩的后门攻击策略：通用攻击激活和压缩适配攻击，以确保恶意触发器信息在压缩后的数据中得以恢复。


<details>
  <summary>Details</summary>
Motivation: 在大数据时代，不可避免的损失性压缩对看不见的后门攻击构成了根本性挑战。传统的方法往往忽略压缩过程，使得后门攻击在传输过程中失效。因此，研究者强调了在设计后门攻击时需要明确考虑压缩过程。

Method: 研究基于区域兴趣（ROI）编码机制，设计了两种新的后门攻击策略：1. 通用攻击激活，这是一个通用方法，使用样本特定的ROI掩膜来重新激活二进制数据流中的触发器信息，适用于已学习图像压缩（LIC）。2. 压缩适配攻击，这是一个创新的攻击策略，使用定制的ROI掩膜将触发器信息编码到二进制数据流中，适用于传统编解码器和LIC。

Result: 实验结果表明，这两种策略在传统编解码器与LIC之间均有效，并验证了它们的有效性。

Conclusion: 本文强调了在设计后门攻击时对压缩过程的考虑，并提供了两种新的攻击策略来确保后门攻击在传输过程中仍然有效。
...
### [11] [CONFETTY: A Tool for Enforcement and Data Confidentiality on Blockchain-Based Processes](https://arxiv.org/abs/2603.13900)
*Michele Kryston,Edoardo Marangone,Alessandro Marcelletti,Claudio Di Ciccio*

Main category: cs.CR

TL;DR: CONFETTY 是一个基于公开区块链的开源平台，旨在保护数据的机密性同时保持操作透明性。通过使用智能合约来实现、执行并存储公开交互，并采用属性基加密技术以实现对敏感信息的细颗粒度访问控制。


<details>
  <summary>Details</summary>
Motivation: 当前采用区块链技术的解决方案虽能提供安全、抵御篡改和透明性，但在需要机密性的场景下则受到限制，因公开区块链环境下难以确保数据的私密性。CONFETTY 提出了新的解决思路，以确保既能充分利用透明优势，又不会泄露敏感信息。

Method: CONFETTY 采用智能合约来执行业务逻辑，并通过密钥制度来控制访问公共交易，同时采用基于属性的加密技术来提供细粒度的隐私保护。

Result: CONFETTY 平台的成功实现证明了可以在保持运营透明度的同时保护敏感数据的机密性，为在区块链上运行安全性高且隐私保护的 PAISs 提供了新方案。

Conclusion: CONFETTY 的研究为在商业环境中应用区块链技术带来了一种新的可能，使得在保障信息公开透明的同时，能够在需要的地方实现隐私保护。
...
### [12] [On secret sharing from extended norm-trace curves](https://arxiv.org/abs/2603.14009)
*Olav Geil*

Main category: cs.CR

TL;DR: 本文进一步研究了从扩展迹曲线的一点代数几何码构建的秘密共享方案，展示了这些方案具有良好的参数和额外的安全层。此外，本文表明，先前使用的方法并非新的竞争性方法，而是对现有理论的一种巧妙应用。


<details>
  <summary>Details</summary>
Motivation: 由于前文提出的方法尚不确定是否优于处理扩展迹曲线相关代码的通用方法，本文旨在更深入地检查并确认前者作为后者的巧妙运用。

Method: 方法是基于增强的戈帕界，在之前作者的文章 [12] 中确立和加强了这一概念。研究基于一重几何代数码和扩展迹曲线的秘密共享方案。

Result: 研究表明，构建自扩展迹曲线的一点代数几何码的秘密共享方案具有良好的参数，并表明从前文使用的方法实际上是增强戈帕界在现有理论中的巧妙应用。

Conclusion: 综上所述，本文的结论是这种构建秘密共享方案的方法是有效的，同时也提供了额外的安全层。
...
### [13] [Missing Mass for Differentially Private Domain Discovery](https://arxiv.org/abs/2603.14016)
...
### [14] [Towards Agentic Honeynet Configuration](https://arxiv.org/abs/2603.14122)
...
### [15] [Membership Inference for Contrastive Pre-training Models with Text-only PII Queries](https://arxiv.org/abs/2603.14222)
...
### [16] [Activation Surgery: Jailbreaking White-box LLMs without Touching the Prompt](https://arxiv.org/abs/2603.14278)
...
### [17] [Governing Dynamic Capabilities: Cryptographic Binding and Reproducibility Verification for AI Agent Tool Use](https://arxiv.org/abs/2603.14332)
*Ziling Zhou*
...
### [18] [Generation of Human Comprehensible Access Control Policies from Audit Logs](https://arxiv.org/abs/2603.14341)
...
### [19] [Toward Secure Web to ERP Payment Flows: A Case Study of HTTP Header Trust Failures in SAP Based Systems](https://arxiv.org/abs/2603.14365)
*Vick Dini*
...
### [20] [Oblivis: A Framework for Delegated and Efficient Oblivious Transfer](https://arxiv.org/abs/2603.14492)
...
### [21] [When Scanners Lie: Evaluator Instability in LLM Red-Teaming](https://arxiv.org/abs/2603.14633)
...
### [22] [$p^2$RAG: Privacy-Preserving RAG Service Supporting Arbitrary Top-$k$ Retrieval](https://arxiv.org/abs/2603.14778)
...
### [23] [Architecture-Agnostic Feature Synergy for Universal Defense Against Heterogeneous Generative Threats](https://arxiv.org/abs/2603.14860)
...
### [24] [Fine-tuning RoBERTa for CVE-to-CWE Classification: A 125M Parameter Model Competitive with LLMs](https://arxiv.org/abs/2603.14911)
*Nikita Mosievskiy*
...
### [25] [Rethinking LLM Watermark Detection in Black-Box Settings: A Non-Intrusive Third-Party Framework](https://arxiv.org/abs/2603.14968)
